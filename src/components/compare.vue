<template>
  <div class="box">
    <div class="titleBox">
      <span>客户案例</span>
      <div class="line"></div>
    </div>
    <div class="compareBox">
      <div class="compareItem wow fadeInDown" data-wow-duration="0.8s">
        <div class="icons">
          <svg
            t="1562234618497"
            class="icon"
            viewBox="0 0 1195 1024"
            version="1.1"
            xmlns="http://www.w3.org/2000/svg"
            p-id="2014"
            width="48"
            height="48"
          >
            <path
              d="M1125.400405 972.814933 651.636395 972.814933 651.636395 631.106304C651.636395 491.702016 663.719168 382.333696 687.885227 302.997931 712.0512 223.662165 737.181781 170.700459 802.935893 107.231829 868.68992 43.763285 914.312789 24.717568 1010.828373 0L1078.255957 145.030059C1010.828373 170.796971 969.749333 182.289237 906.391381 248.024235 843.033429 313.759232 838.360149 377.340075 838.360149 431.988736L1125.400405 431.988736 1125.400405 972.814933ZM473.764096 972.814933 0 972.814933 0 631.106304C0 491.702016 12.082859 382.333696 36.248832 302.997931 60.414891 223.662165 85.545472 170.700459 151.299499 107.231829 217.053611 43.763285 262.676395 24.717568 359.192064 0L426.619563 145.030059C359.192064 170.796971 318.112939 182.289237 254.754987 248.024235 191.397035 313.759232 186.723755 377.340075 186.723755 431.988736L473.764096 431.988736 473.764096 972.814933Z"
              p-id="2015"
              data-spm-anchor-id="a313x.7781069.0.i0"
              class
              fill="#8a8a8a"
            />
          </svg>
        </div>
        <img class="avatar" src="https://img.linghegl.cn/juzi.png" />
        <div class="content">
          <span>作为一家聊天机器人公司，Authing 身份管家小程序使我们一年的短信费率降低了至少50%，它简化了登录流程，很好地解决了短信API被盗刷的问题。</span>
          <div class="companyName">—— 句子互动</div>
        </div>
      </div>

      <div class="compareItem centerItem wow fadeInDown" data-wow-duration="1.2s">
        <div class="icons">
          <svg
            t="1562234618497"
            class="icon"
            viewBox="0 0 1195 1024"
            version="1.1"
            xmlns="http://www.w3.org/2000/svg"
            p-id="2014"
            width="48"
            height="48"
          >
            <path
              d="M1125.400405 972.814933 651.636395 972.814933 651.636395 631.106304C651.636395 491.702016 663.719168 382.333696 687.885227 302.997931 712.0512 223.662165 737.181781 170.700459 802.935893 107.231829 868.68992 43.763285 914.312789 24.717568 1010.828373 0L1078.255957 145.030059C1010.828373 170.796971 969.749333 182.289237 906.391381 248.024235 843.033429 313.759232 838.360149 377.340075 838.360149 431.988736L1125.400405 431.988736 1125.400405 972.814933ZM473.764096 972.814933 0 972.814933 0 631.106304C0 491.702016 12.082859 382.333696 36.248832 302.997931 60.414891 223.662165 85.545472 170.700459 151.299499 107.231829 217.053611 43.763285 262.676395 24.717568 359.192064 0L426.619563 145.030059C359.192064 170.796971 318.112939 182.289237 254.754987 248.024235 191.397035 313.759232 186.723755 377.340075 186.723755 431.988736L473.764096 431.988736 473.764096 972.814933Z"
              p-id="2015"
              data-spm-anchor-id="a313x.7781069.0.i0"
              class
              fill="#8a8a8a"
            />
          </svg>
        </div>
        <img class="avatar" src="https://img.linghegl.cn/gaojiaoshe.png" />
        <div class="content">
          <span>把登录与身份认证交给 Authing 后，我们的工程师能够更加专注于产品本身。Authing 的用户管理后台简明专业，收费相对其他服务商也更便宜。</span>
          <div class="companyName">—— 高等教育出版社</div>
        </div>
      </div>

      <div class="compareItem wow fadeInDown" data-wow-duration="1.6s">
        <!--  -->
        <div class="icons">
          <svg
            t="1562234618497"
            class="icon"
            viewBox="0 0 1195 1024"
            version="1.1"
            xmlns="http://www.w3.org/2000/svg"
            p-id="2014"
            width="48"
            height="48"
          >
            <path
              d="M1125.400405 972.814933 651.636395 972.814933 651.636395 631.106304C651.636395 491.702016 663.719168 382.333696 687.885227 302.997931 712.0512 223.662165 737.181781 170.700459 802.935893 107.231829 868.68992 43.763285 914.312789 24.717568 1010.828373 0L1078.255957 145.030059C1010.828373 170.796971 969.749333 182.289237 906.391381 248.024235 843.033429 313.759232 838.360149 377.340075 838.360149 431.988736L1125.400405 431.988736 1125.400405 972.814933ZM473.764096 972.814933 0 972.814933 0 631.106304C0 491.702016 12.082859 382.333696 36.248832 302.997931 60.414891 223.662165 85.545472 170.700459 151.299499 107.231829 217.053611 43.763285 262.676395 24.717568 359.192064 0L426.619563 145.030059C359.192064 170.796971 318.112939 182.289237 254.754987 248.024235 191.397035 313.759232 186.723755 377.340075 186.723755 431.988736L473.764096 431.988736 473.764096 972.814933Z"
              p-id="2015"
              data-spm-anchor-id="a313x.7781069.0.i0"
              class
              fill="#8a8a8a"
            />
          </svg>
        </div>
        <img class="avatar" src="https://img.linghegl.cn/wandian.gif" />
        <div class="content">
          <span>Authing 使我们的开发人员能够更轻松地完成复杂的工作。接入小程序端后，我们能平滑地渡过初创阶段，快速积累用户量，管理和分析用户数据。</span>
          <div class="companyName">—— 玩店</div>
        </div>
      </div>
    </div>
  </div>
</template>
<script>
export default {
  name: "compare",
  components: {}
};
</script>

<style lang="less">
.avatar {
  width: 80px;
  height: 80px;
  border-radius: 50%;
  box-shadow: 0 2px 1px -1px rgba(0, 0, 0, 0.2), 0 1px 1px 0 rgba(0, 0, 0, 0.14),
    0 1px 3px 0 rgba(0, 0, 0, 0.12);
}

.compareBox {
  width: 100%;
  height: 70%;
  display: flex;
  flex-direction: row;
  justify-content: space-around;
  align-items: center;
}

.compareItem {
  width: 32vw;
  height: 60vh;
  // margin: 0 1vw;

  background: #fffefe;
  border-radius: 10px;

  padding: 44px;

  display: flex;
  flex-direction: column;
  justify-content: flex-start;
  align-items: center;
}

.centerItem {
  margin: 0 2vw;
}

.content {
  width: 100%;
  height: calc(100% - 80px - 24px);
  font-size: 20px;
  color: #9a9a9a;
  margin-top: 24px;
  line-height: 1.5;
  font-weight: 500;
  text-indent: 2em;
}

.content .companyName {
  text-indent: unset !important;
  margin-top: 33px;
  text-align: right;
  font-size: 20px;
}

@media screen and (max-width: 720px) {
  .compareBox {
    display: block !important;
    height: unset !important;
  }

  .compareItem {
    width: 100%;
    height: 40vh;
    margin-bottom: 22px !important;
    // margin: 0 1vw;

    background: #fffeff;
    border-radius: 10px;

    padding: 22px;
  }

  .centerItem {
    margin: 0 !important;
    margin-bottom: 22px !important;
  }

  .avatar {
    width: 68px;
    height: 68px;
    border-radius: 50%;
    box-shadow: 0 2px 1px -1px rgba(0, 0, 0, 0.2),
      0 1px 1px 0 rgba(0, 0, 0, 0.14), 0 1px 3px 0 rgba(0, 0, 0, 0.12);
  }

  .content {
    width: 100%;
    height: calc(100% - 68px - 24px);
    font-size: 16px;
    color: #9a9a9a;
    margin-top: 24px;
    line-height: 1.8;
    font-weight: 500;
  }

  .content .companyName {
    text-indent: unset !important;
    margin-top: 11px !important;
    text-align: right;
    font-size: 14px !important;
  }
}

.compareLeft {
  margin-right: 2vw;
}

.icons {
  position: absolute !important;
  z-index: 2;
  justify-self: flex-start !important;
  align-self: flex-start !important;
}

@keyframes shakeTopx {
  0%,
  100% {
    -webkit-transform: rotate(0deg);
    transform: rotate(0deg);
    -webkit-transform-origin: 50% 0;
    transform-origin: 50% 0;
  }
  5% {
    -webkit-transform: rotate(2deg);
    transform: rotate(2deg);
  }
  10%,
  20%,
  30% {
    -webkit-transform: rotate(-4deg);
    transform: rotate(-4deg);
  }
  15%,
  25%,
  35% {
    -webkit-transform: rotate(4deg);
    transform: rotate(4deg);
  }
  40% {
    -webkit-transform: rotate(-2deg);
    transform: rotate(-2deg);
  }
  45% {
    -webkit-transform: rotate(2deg);
    transform: rotate(2deg);
  }
  50% {
    -webkit-transform: rotate(0deg);
    transform: rotate(0deg);
  }
}
</style>
